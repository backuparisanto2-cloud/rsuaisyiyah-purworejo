...
const empty: Omit<Slide, "id"> = {
  image_url: "", title: "", subtitle: "", cta_text: "", cta_link: "",
...
function HeroSliderAdmin() {
  ...
  async function save() {
    if (!editing) return;
    if (!editing.image_url) { toast.error("Gambar wajib"); return; }
    const payload = {
      image_url: editing.image_url, title: editing.title, subtitle: editing.subtitle,
      cta_text: editing.cta_text, cta_link: editing.cta_link,
  ...
  async function onReorder(next: Slide[]) {
    setSlides(next);
    await persistOrder("hero_slides", next, supabase);
    void load();
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? <p className="text-muted-foreground">Memuat...</p>
        : slides.length === 0 ? <Card className="p-8 text-center text-muted-foreground">Belum ada slide.</Card>
        : (
          <SortableList
            items={slides}
            onReorder={onReorder}
        ...
        <DialogContent className="max-w-lg">
          ...
          {editing && (
            <div className="space-y-3">
              <div><Label>Gambar *</Label><ImageUpload value={editing.image_url} onChange={(url) => setEditing({ ...editing, image_url: url ?? "" })} folder="hero" /></div>
              <div><Label>Judul</Label><Input value={editing.title ?? ""} onChange={(e) => setEditing({ ...editing, title: e.target.value })} /></div>
              <div><Label>Subjudul</Label><Textarea rows={2} value={editing.subtitle ?? ""} onChange={(e) => setEditing({ ...editing, subtitle: e.target.value })} /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><Label>Teks CTA</Label><Input value={editing.cta_text ?? ""} onChange={(e) => setEditing({ ...editing, cta_text: e.target.value })} /></div>
                <div><Label>Link CTA</Label><Input value={editing.cta_link ?? ""} onChange={(e) => setEditing({ ...editing, cta_link: e.target.value })} /></div>
              </div>
